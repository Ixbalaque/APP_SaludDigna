...
def enviar_mensaje():
    ...
    per = Persona.query.filter_by(usuario=key_param).first()
       
    if ( per.check_password(contrasena) ):
   
        if per:
            if (nombre or email or usuario ):
                ...
                return jsonify({
                    'mensaje': 'Sin Modificaciones en la Cuenta.',
                    'nombre':  per.nombre,
                    'usuario': per.usuario,
                    'correo':  per.email
                })
