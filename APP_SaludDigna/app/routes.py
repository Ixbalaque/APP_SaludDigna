...
def enviar_mensaje():
    ...
    return {'mensaje': f'Datos del usuario  actualizados exitosamente.'}
"""
...
def enviar_mensaje():
    ...
    key_param = request.form.getlist('key_R')[1]
    nombre    = request.form.getlist('nombre_R')
    email     = request.form.getlist('correo_R')
    persona = Persona.query.filter_by(usuario=key_param)
    
    print(request.form)  # Imprimir el contenido del formulario en la consola del servidor

  
    if persona:
        # Actualizar los datos del usuario según lo que recibiste en el formulario 
        persona.nombre = 'Jose'

        # Guardar los cambios en la base de datos
        db.session.commit()

        return jsonify({'mensaje': f'Datos del usuario {key_param} actualizados exitosamente.'})
    else:
        return jsonify({'mensaje': f'Usuario {key_param} no encontrado.'})
